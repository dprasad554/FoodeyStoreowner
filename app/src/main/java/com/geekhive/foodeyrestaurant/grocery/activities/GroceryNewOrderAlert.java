...
public class GroceryNewOrderAlert extends AppCompatActivity implements OnResponseListner {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        slide_to_accept.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                //staring the player
                player.stop();
                CallOrderService(storeNewOrder.getOrder().get(position_).getGCart().getId(), storeNewOrder.getOrder().get(position_).getGCart().getStoreId());
            }
        });
    ...
    private void CallService() {
        String store_id = PrefsGrocery.getUserIdGrocery(this);
    ...
    @Override
    public void onResponse(Object response, com.geekhive.foodeyrestaurant.grocery.utils.WebServices.ApiType apiType, boolean isSuccess) {
        if (apiType == WebServices.ApiType.storeneworder) {
            if (!isSuccess) {
                com.geekhive.foodeyrestaurant.grocery.utils.SnackBar.makeText(this, getResources().getString(R.string.something_wrong), com.geekhive.foodeyrestaurant.grocery.utils.SnackBar.LENGTH_SHORT).show();
            } else {
                ...
                if (!isSuccess || storeNewOrder == null) {
                    ...
                    if (storeNewOrder != null){
                        if (storeNewOrder.getOrder() != null){
                            final String name = storeNewOrder.getOrder().get(position_).getUser().getFirstname() + " " + storeNewOrder.getOrder().get(position_).getUser().getLastname()
                                    + "\n" + storeNewOrder.getOrder().get(position_).getUser().getMobile() + "\n \n" + storeNewOrder.getOrder().get(position_).getGCart().getOrderId();
                            storeName.setText(name);
                        } else {
                            SnackBar.makeText(this, "Record Not Found", com.geekhive.foodeyrestaurant.grocery.utils.SnackBar.LENGTH_SHORT).show();
                        }
                    } else {
            ...
            com.geekhive.foodeyrestaurant.grocery.beans.groceryorderConfirm.OrderConfirmation orderConfirmation = (OrderConfirmation) response;
