...
import com.geekhive.foodeyrestaurant.grocery.activities.Grocery_NewOrderDetailsActivity;
import com.geekhive.foodeyrestaurant.grocery.beans.grocerystoreneworder.StoreNewOrder;

public class Grocery_HomeListAdapter extends RecyclerView.Adapter<Grocery_HomeListAdapter.MyViewHolder> {

    Context context;
    StoreNewOrder storeNewOrder;

    public Grocery_HomeListAdapter(Context context, StoreNewOrder storeNewOrder ) {
    ...
    public void onBindViewHolder(MyViewHolder holder, final int position) {
        ...
        holder.orderId.setText(storeNewOrder.getOrder().get(position).getGCart().getOrderId());
        holder.orderDate.setText(storeNewOrder.getOrder().get(position).getGCart().getDate());

        holder.viewOrder.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent intent = new Intent(context, Grocery_NewOrderDetailsActivity.class);
                intent.putExtra("position", position);
                intent.putExtra("cart_id",storeNewOrder.getOrder().get(position).getGCart().getId());
